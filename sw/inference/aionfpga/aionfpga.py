...
def sine_squared_window(num_frames, num_frames_considered):
    window = np.zeros((fh.frames_to_consider,), dtype=np.float32)
    for n in range(num_frames_considered):
...
def main():
    ...
    while True:
        ...
        weighted_prediction = np.matmul(window, predictions) / np.sum(window) # computation of the weighted prediction
        ...
        frame_ui = frame_ui_png.tobytes() # the frame
